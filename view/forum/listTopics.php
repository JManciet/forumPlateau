...
foreach($topics as $topic ){
    ?>
    <p>
        <div onclick="location.href='index.php?ctrl=forum&action=paginationPosts&page=1&id=<?=$topic->getId()?>';" class="topicContainer">
            <div class="topicTitle">
                <div><a href="index.php?ctrl=forum&action=paginationPosts&page=1&id=<?=$topic->getId()?>"><?=$topic->getTitle()?></a></div>
...
<nav>
    <ul class="pagination">
        ...
        <li class="page-item <?= ($currentPage == 1) ? "disabled" : "" ?>">
            <a href="index.php?ctrl=forum&action=paginationTopics&page=<?= $currentPage - 1 ?>" class="page-link">Précédente</a>
        </li>
        <?php for($page = 1; $page <= $totalPages; $page++): ?>
            <!-- Lien vers chacune des pages (activé si on se trouve sur la page correspondante) -->
            <li class="page-item <?= ($currentPage == $page) ? "active" : "" ?>">
                <a href="index.php?ctrl=forum&action=paginationTopics&page=<?= $page ?>" class="page-link"><?= $page ?></a>
            </li>
        <?php endfor ?>
            <!-- Lien vers la page suivante (désactivé si on se trouve sur la dernière page) -->
            <li class="page-item <?= ($currentPage == $totalPages) ? "disabled" : "" ?>">
            <a href="index.php?ctrl=forum&action=paginationTopics&page=<?= $currentPage + 1 ?>" class="page-link">Suivante</a>
        </li>
    </ul>
</nav>
